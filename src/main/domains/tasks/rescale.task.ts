...
export async function execute(input: TrimmedImage): Promise<RescaledImage> {
  ...
  try {
    ...
    const borderSize = input.settings.borderSize;
    ...
    const availableWidth = canvasWidth - borderSize * 2;
    const availableHeight = canvasHeight - borderSize * 2;
    ...
    const borderedWidth = scaledWidth + borderSize * 2;
    const borderedHeight = scaledHeight + borderSize * 2;
    ...
    await pipeline
      .extend({
        top: paddingTop + borderSize,
        bottom: paddingBottom + borderSize,
        left: paddingLeft + borderSize,
        right: paddingRight + borderSize,
        background: { r: 0, g: 0, b: 0, alpha: 0 }
      })
    ...
    console.error(`[rescale] ERROR: ${error instanceof Error ? error.message : String(error)}`);
    console.error(`[rescale] Stack: ${error instanceof Error ? error.stack : 'N/A'}`);
    throw error;
  }
}
