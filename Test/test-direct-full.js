...
async function testFile(inputFile, outputDir, ffmpegBin) {
  ...
  try {
    ...
    // Создаём команду для всех 4 тайлов одновременно
    const tiles = [];
    ...
    // Строим команду FFmpeg с множественными выходами
    let cmd = `${ffmpegBin} -y -loop 1 -framerate ${originalFps} -i "${atlasPath}"`;
    
    for (const tile of tiles) {
      cmd += ` -vf "crop=${tileSize}:${tileSize}:${tile.x}:'n*${frameHeight}',format=yuva420p" -frames:v ${frameCount} -c:v libvpx-vp9 -pix_fmt yuva420p -auto-alt-ref 0 -crf 45 -b:v 0 -deadline realtime -cpu-used 6 -row-mt 1 -an "${tile.path}"`;
    }
...
async function main() {
  console.log('=== Direct PNG Atlas → WebM Tiles Test ===\n');
