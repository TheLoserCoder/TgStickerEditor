#!/usr/bin/env node

const path = require('path');
const fs = require('fs').promises;
const { exec } = require('child_process');
const { promisify } = require('util');
const execAsync = promisify(exec);

async function testFragmentation() {
  console.log('=== FFmpeg Fragment Test ===\n');
  
  const inputGif = path.join(__dirname, 'Asset', 'rickroll-roll.gif');
  const outputDir = path.join(__dirname, 'output');
  const tempScaled = path.join(outputDir, 'temp_scaled.gif');
  
  try {
    // Проверяем входной файл
    await fs.access(inputGif);
    console.log(`✓ Input file found: ${inputGif}\n`);
    
    // Шаг 1: Масштабируем GIF до 1024x1024
    console.log('Step 1: Scaling GIF to 1024x1024...');
    await execAsync(`ffmpeg -y -i "${inputGif}" -vf "scale=1024:1024:flags=lanczos" "${tempScaled}"`);
    console.log('✓ Scaled GIF\n');
    
    // Шаг 2: Получаем метаданные
    console.log('Step 2: Getting metadata...');
    const { stdout: probeOutput } = await execAsync(`ffprobe -v quiet -print_format json -show_format -show_streams "${tempScaled}"`);
    const metadata = JSON.parse(probeOutput);
    const videoStream = metadata.streams.find(s => s.codec_type === 'video');
    console.log(`✓ Size: ${videoStream.width}x${videoStream.height}`);
    console.log(`✓ Duration: ${metadata.format.duration}s\n`);
    
    // Шаг 3: Фрагментация через FFmpeg
    console.log('Step 3: Fragmenting to WebM tiles (2x2 grid)...');
    
    const duration = parseFloat(metadata.format.duration);
    const maxDuration = 2.99;
    const setptsFactor = duration > maxDuration ? maxDuration / duration : 1;
    const fps = 30;
    
    // Строим filter_complex для 2x2 сетки
    const filterComplex = `
[0:v]fps=${fps},setpts=${setptsFactor}*PTS,split=4[v0][v1][v2][v3];
[v0]crop=512:512:0:0[t0];
[v1]crop=512:512:512:0[t1];
[v2]crop=512:512:0:512[t2];
[v3]crop=512:512:512:512[t3]
`.trim();
    
    const ffmpegCmd = `ffmpeg -y -i "${tempScaled}" -filter_complex "${filterComplex}" \\
      -map "[t0]" -c:v libvpx-vp9 -pix_fmt yuva420p -auto-alt-ref 0 -crf 40 -b:v 0 -deadline realtime -cpu-used 6 -row-mt 1 -an "${outputDir}/tile_0_0.webm" \\
      -map "[t1]" -c:v libvpx-vp9 -pix_fmt yuva420p -auto-alt-ref 0 -crf 40 -b:v 0 -deadline realtime -cpu-used 6 -row-mt 1 -an "${outputDir}/tile_0_1.webm" \\
      -map "[t2]" -c:v libvpx-vp9 -pix_fmt yuva420p -auto-alt-ref 0 -crf 40 -b:v 0 -deadline realtime -cpu-used 6 -row-mt 1 -an "${outputDir}/tile_1_0.webm" \\
      -map "[t3]" -c:v libvpx-vp9 -pix_fmt yuva420p -auto-alt-ref 0 -crf 40 -b:v 0 -deadline realtime -cpu-used 6 -row-mt 1 -an "${outputDir}/tile_1_1.webm"`;
    
    await execAsync(ffmpegCmd);
    console.log('✓ Fragmentation complete!\n');
    
    // Шаг 4: Проверяем результаты
    console.log('Step 4: Checking output files...\n');
    
    const tiles = [
      'tile_0_0.webm',
      'tile_0_1.webm',
      'tile_1_0.webm',
      'tile_1_1.webm'
    ];
    
    for (const tile of tiles) {
      const tilePath = path.join(outputDir, tile);
      const stats = await fs.stat(tilePath);
      
      // Проверяем метаданные тайла
      const { stdout: tileProbe } = await execAsync(`ffprobe -v quiet -print_format json -show_format -show_streams "${tilePath}"`);
      const tileMetadata = JSON.parse(tileProbe);
      const tileStream = tileMetadata.streams.find(s => s.codec_type === 'video');
      
      console.log(`${tile}:`);
      console.log(`  Size: ${(stats.size / 1024).toFixed(2)} KB`);
      console.log(`  Resolution: ${tileStream.width}x${tileStream.height}`);
      console.log(`  Codec: ${tileStream.codec_name}`);
      console.log(`  Pixel Format: ${tileStream.pix_fmt}`);
      console.log(`  Duration: ${tileMetadata.format.duration}s`);
      console.log(`  Has Alpha: ${tileStream.pix_fmt.includes('yuva') ? 'YES ✓' : 'NO ✗'}`);
      console.log(`  Frames: ${tileStream.nb_frames || 'N/A'}`);
      console.log();
    }
    
    console.log('=== Test completed successfully ===');
    console.log(`Output directory: ${outputDir}`);
    
  } catch (error) {
    console.error('\n=== Test failed ===');
    console.error(error.message);
    if (error.stderr) {
      console.error('\nFFmpeg stderr:');
      console.error(error.stderr);
    }
    process.exit(1);
  }
}

testFragmentation();
